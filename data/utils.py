...
def read_data(path: str, nrows=None) -> pd.DataFrame:
    ...
    # Business datasets are commonly stored as a wide table such as
    # ``time,power,ws`` instead of the benchmark's long ``value,cols`` format.
    if "cols" not in data.columns:
        time_column = next(
            (column for column in ("time", "date") if column in data.columns), None
        )
        if time_column is None:
            raise ValueError(
                "Wide-format data must contain a 'time' or 'date' column"
            )
        data[time_column] = pd.to_datetime(data[time_column], errors="raise")
        data = data.set_index(time_column).sort_index()
        if data.index.has_duplicates:
            raise ValueError("Time column contains duplicate timestamps")
        if nrows is not None and isinstance(nrows, int) and len(data) >= nrows:

...
def collect_business_issue_indices(
    dataset: pd.DataFrame,
    issue_hour: int,
    horizon: int,
    seq_len: int,
) -> List[int]:
    """Collect row positions whose timestamp is a valid day-ahead issue time."""
    if not isinstance(dataset.index, pd.DatetimeIndex):
        raise ValueError("Business training requires a datetime index")

    indices = []
    for pos in range(seq_len, len(dataset) - horizon + 1):
        ts = dataset.index[pos]
        if ts.hour != issue_hour or ts.minute != 0:
            continue
        indices.append(pos)
    return indices


def split_business_issue_indices(
    ...
    def _validate_indices(indices: List[int], split_name: str) -> None:
        for pos in indices:
            if pos < seq_len:
                raise ValueError(
                    f"{split_name} issue index {pos} does not have enough history "
                    f"(requires {seq_len} points before issue time)"
                )
            if pos + horizon > dataset_len:
                raise ValueError(
                    f"{split_name} issue index {pos} does not have enough future "
                    f"horizon (requires {horizon} points after issue time)"
                )
    ...
    if ratio == 1:
        _validate_indices(issue_indices, "train")
        return issue_indices, []

    border = int(len(issue_indices) * ratio)
    if border <= 0 or border >= len(issue_indices):
        raise ValueError(
            "train_ratio_in_tv leaves no business day-ahead samples for train or valid"
        )

    train_indices = issue_indices[:border]
    valid_indices = issue_indices[border:]
    _validate_indices(train_indices, "train")
    _validate_indices(valid_indices, "valid")
    return train_indices, valid_indices


def collect_business_scaler_fit_data(
    dataset: pd.DataFrame,
    train_issue_indices: List[int],
    seq_len: int,
) -> pd.DataFrame:
    """Collect historical windows for train issue times to fit scalers without leakage."""
    if not train_issue_indices:
        raise ValueError("No train issue indices are available for scaler fitting")

    history_frames = [
        dataset.iloc[pos - seq_len: pos] for pos in train_issue_indices
    ]
    scaler_fit_data = pd.concat(history_frames)
    return scaler_fit_data[~scaler_fit_data.index.duplicated(keep="first")]


def build_business_valid_data(
    dataset: pd.DataFrame,
    valid_issue_indices: List[int],
    seq_len: int,
    horizon: int,
) -> Tuple[pd.DataFrame, List[int]]:
    """Slice validation data and rebase valid issue indices to the local frame."""
    valid_start = valid_issue_indices[0] - seq_len
    valid_end = valid_issue_indices[-1] + horizon
    valid_data = dataset.iloc[valid_start:valid_end]
    valid_issue_indices_local = [pos - valid_start for pos in valid_issue_indices]
    return valid_data, valid_issue_indices_local


class BusinessDayAheadDataset:
    """Build samples aligned with the business day-ahead evaluation strategy."""
    ...
    def __init__(
        ...
        self.dataset = dataset
        self.issue_indices = issue_indices
    ...
    def __getitem__(self, index):
        issue_pos = self.issue_indices[index]
        s_begin = issue_pos - self.history_length
        s_end = issue_pos
        r_begin = s_end
        r_end = r_begin + self.prediction_length

        seq_x = self.dataset.iloc[s_begin:s_end]
        seq_y = self.dataset.iloc[r_begin:r_end]
...
def business_day_ahead_data_provider(
    data,
    config,
    timeenc,
    batch_size,
    ...
    if issue_indices is None:
        issue_indices = collect_business_issue_indices(
            data,
            config.issue_hour,
            config.horizon,
            config.seq_len,
    ...
    if not issue_indices:
        raise ValueError("No valid business day-ahead training windows were found")

    dataset = BusinessDayAheadDataset(
        dataset=data,

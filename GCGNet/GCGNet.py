...
from ts_benchmark.baselines.GCGNet.utils.utils import (
    ...
    business_day_ahead_data_provider,
    collect_business_issue_indices,
    collect_business_scaler_fit_data,
    build_business_valid_data,
    split_business_issue_indices,
    train_val_split,
    get_time_mark,
...
class GCGNet(ModelBase):
    ...
    def _process(self, input, target, input_mark, target_mark, exog_future=None):
        ...
        if exog_future is not None and not self.config.use_future_exog:
            exog_future = torch.zeros_like(exog_future)
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                target = target[:, -self.loss_len:, :target_dim]
                output = output[:, -self.loss_len:, :target_dim]
                if (
    ...
    def forecast_fit(
        ...
        # Ensure target_dim does not exceed series_dim
        if not hasattr(self.config, 'target_dim') or self.config.target_dim is None:
            self.config.target_dim = series_dim
        self.config.target_dim = min(self.config.target_dim, series_dim)
        if self.config.business_train:
            if self.config.evaluation_horizon > self.config.horizon:
                raise ValueError(
                    "evaluation_horizon must not exceed horizon during business training"
                )
            self.loss_len = self.config.evaluation_horizon
        else:
            self.loss_len = self.model_pred_len
        ...
        config = self.config
        valid_issue_indices: list = []
        if config.business_train:
            all_issue_indices = collect_business_issue_indices(
                train_valid_data,
                config.issue_hour,
                config.horizon,
                config.seq_len,
            )
            if not all_issue_indices:
                raise ValueError("No valid business day-ahead training windows were found")

            train_issue_indices, valid_issue_indices = split_business_issue_indices(
                all_issue_indices,
                train_ratio_in_tv,
            ...
            scaler_fit_data = collect_business_scaler_fit_data(
                train_valid_data, train_issue_indices, config.seq_len
            )
            logger.info(
                "Business training enabled: %s train windows, %s valid windows",
                len(train_issue_indices),
                len(valid_issue_indices),
            )
        else:
        ...
        if exog_dim > 0:
            self.scaler1.fit(scaler_fit_data.values[:, :series_dim])
            self.scaler2.fit(scaler_fit_data.values[:, series_dim:])
            if config.norm:
                scaled_series = self.scaler1.transform(
                    train_valid_data.values[:, :series_dim]
                )
                scaled_exog = self.scaler2.transform(
                    train_valid_data.values[:, series_dim:]
                )
                scaled_train_valid_data = pd.DataFrame(
                    np.concatenate((scaled_series, scaled_exog), axis=1),
                    columns=train_valid_data.columns,
                    index=train_valid_data.index,
                )
            else:
                scaled_train_valid_data = train_valid_data.copy()
        else:
            self.scaler1.fit(scaler_fit_data.values)
            if config.norm:
                scaled_train_valid_data = pd.DataFrame(
                    self.scaler1.transform(train_valid_data.values),
                    columns=train_valid_data.columns,
                    index=train_valid_data.index,
                )
            else:
        ...
        if config.business_train:
            train_dataset, self.train_data_loader = business_day_ahead_data_provider(
                scaled_train_valid_data,
                config,
                timeenc=1,
                batch_size=config.batch_size,
                shuffle=True,
                drop_last=train_drop_last,
                issue_indices=train_issue_indices,
            )
            if valid_issue_indices:
                valid_data, valid_issue_indices_local = build_business_valid_data(
                    scaled_train_valid_data,
                    valid_issue_indices,
                    config.seq_len,
                    config.horizon,
                )
                _, valid_data_loader = business_day_ahead_data_provider(
                    valid_data,
                    config,
                    timeenc=1,
                    batch_size=config.batch_size,
                    shuffle=False,
                    drop_last=False,
                    issue_indices=valid_issue_indices_local,
                )
        else:
            if config.norm:
                if exog_dim > 0:
        ...
        for epoch in range(config.num_epochs):
            ...
            if train_ratio_in_tv != 1 and (
                not config.business_train or valid_issue_indices
            ):
    ...
    def batch_forecast(
        ...
        return answers[..., :self.config.target_dim]
